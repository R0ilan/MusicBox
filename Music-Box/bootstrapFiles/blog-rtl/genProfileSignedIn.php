...
<head>
    ...
    jQuery(document).ready(function($) {
        ...
        $.ajax({
            ...
            success: function(response) {
            if (response) {
                for (review of response['reviews']) {
                    let divReview = document.createElement('div');

                    let imgAlbum = document.createElement('img');

                    if (review['track']['album']['images'].length > 0) {
                        imgAlbum.setAttribute('src', review['track']['album']['images'][0]['url']);
                    }

                    divReview.appendChild(imgAlbum);

                    let pTrackName = document.createElement('p');
                    pTrackName.textContent = review['track']['name'];
                    divReview.appendChild(pTrackName);

                    let pReviewText = document.createElement('p');
                    pReviewText.textContent = review['review_text'];
                    divReview.appendChild(pReviewText);

                    let pReviewTime = document.createElement('p');
                    pReviewTime.textContent = review['review_time'];
                    divReview.appendChild(pReviewTime);

                    $('#getReviewsByUserResult').append(divReview);
                }
            }
            },
        ...
        <div class="container">
            ...
            <div class="right">
                ...
                <div class="reviews">
                    ...
                    <div id="getReviewsByUserResult"></div>
                </div>
            </div>
            <!----------------- END OF RIGHT -------------------->
        </div>
